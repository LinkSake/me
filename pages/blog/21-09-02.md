...
Hay una multitud de factores que pueden contribuir esto, por ello [Docker](queesdockerpuntocom) nos ofrece una solución con la cual podemos tener un mayor control sobre estas variables a través de las computadoras que sean necesarias.  
...
Para poder seguir esta guía necesitaras tener [Docker instalado](instalardockerpuntocom) así como un proyecto el cual quieras *dockerizar*, si solamente quieres prácticar puedes usar [este proyecto de ejemplo](github) el cual necesita una conexión a [Postgres](psql) y a [Redis](redis) para funcionar.

¿Eres impaciente? ¡Puedes clonar [esta rama](github) del proyecto donde ya se encuentran los archivos necesarios para correr el projecto dentro de Docker!

## Primero viene el Dockerfile

Lo primero que haremos será hacer una [imagen](queesunaimagendedockerpuntocom) personalizada para nuestro proyecto, así que crearemos un archivo en la raíz del mismo llamado `Dockerfile`

```bash
touch Dockerfile
```

La primer linea de nuestro archivo definirá la imagen de la cual nos basaremos, en este caso será la [imagen oficial de Ruby](dockerhub), pero usaremos la versión de [Alpine]() para tener una imagen más ligera como resultado.
...
Así como le dimos un hogar a tu proyecto dentro del contenedor que crearemos, las gemas del mismo necesitan una carpeta también. Por ello, le informaremos a [bundler](bundler) donde colocarlas a través de una variable de ambiente.
...
Para poder coordinar todos los servicios que necesitamos para el correcto funcionamiento de nuestra aplicación (en este caso 2 bases de datos: Posgres y Redis) usaremos [docker-compose](dockercompose), esta útilidad de Docker nos ayudará a crear multimples contenedores de diferentes imagenes, [conectarlos](dockernetwork), darles [variables de ambiente](envsdocekr) e incluso [volumenes](dockervolumenes).
...
Finalmente los `volumes` son la infomación persistente que necesitaremos para no correr las migraciones cada vez que encendamos el contenedor, esto porque Docker borra todos los datos una vez que damos de baja la información, si quieres aprender más sobre este tema te recomiendo [esta]() sección de la documentación.

Ahora, el siguiente servicio es el de Redis pero no ahondaremos mucho en el pues cuenta solo con un par de etiquetas las cuales ya hemos revisado, para más información puedes visitar [la imagen oficial.](dockerredis)
...
> ¿Por qué solamente `db:create` y no `rails db:create` o `rake db:create`? En nuestro `Dockerfile` dimos como punto de entrada el comando `rails`, por ello solo es necesario pasar los parametros. Si quisieramos efectuar otro comando dentro del contenedor esto tendría que ser a través de [docker exec](dockerexec).

## Nuestra aplicación en un contenedor

Con la base de datos creada, solo queda un comando que corra los contenedores en [modo separado](dockerdetached) y podremos ver el fruto de nuestro trabajo.
...
Espero que te haya sido útil, cualquier cosa puedes [contactarme](https://luisangel.me/about#contacto) y responderé lo más pronto posible.
